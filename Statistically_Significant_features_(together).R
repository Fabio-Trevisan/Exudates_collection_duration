...
df4$PValue <- pmin(df4$Tr,
                   df4$Time,
                   df4$`Tr:Time`) #find min P Value

#P Value filtering 
z <- 0.05 #filtering ratio
Filtered <- df4[df4$`PValue`<z,] #P_Value filtering >z
...
#save
write.csv(Clean_df, file = paste("Exudates_Significant.csv", sep="_"), row.names=FALSE)
